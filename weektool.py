...
#                                                                              #
#   > WEEK TOOL is a tool designed to grather information about a target         #
#     which is publicly available.                                             #
#   > Usage of `WEEK TOOL` for attacking targets without prior mutual            #
#     consent is illegal.                                                      #
...
def save_output(prv_cmd):
    try:
        filename = prv_cmd+'_WEEK TOOL_'+str(time.strftime("%d-%m-%Y_%H-%M-%S"))+'.txt'
        pwd = os.getcwd()
        os_name = os.uname()
        path = pwd+'/'+filename
...
def control_center():
    ...
    if not silent_mode: printit('[#] Initializing `WEEK TOOL` framework...', coledt=[1, 49, random.choice([92, 96, 97])], line_down=True, line_up=True, space_up=True)
    ...
    \33[1;49;93m> Syntax:\033[0m \33[1;49;97m[<user>@WEEKTOOL]⸺ \33[1;49;96m[<expression>]\33[1;49;97m $ <command>\033[0m
    ...
    |----------------------------------------------------|
    | \33[1;49;97mName\033[0m          | \33[1;49;96mWEEKTOOL\033[0m                            |
    ...
    while True:
        # [user@phomber]⸺[:D] $
        cmd = (printit(f'\n\033[0m[{user.strip()}@WEEK TOOL]⸺ [{crt_exp}] $ {crt_color}', input_mode=True, space_up=True)).strip()
    ...
    \033[0m'''
        ...
        # Basic commands
            ...
            if not silent_mode: printit('    [@] Thank you for using `WEEK TOOL` osint framework :)', coledt=[1, 49, random.choice([92, 96, 97])])
